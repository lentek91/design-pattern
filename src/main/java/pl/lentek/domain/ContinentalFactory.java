...
public class ContinentalFactory extends Factory {
  ...
  private final static SteeringWheelPosition steeringWheelPosition = SteeringWheelPosition.RIGHT;

  @Override
  public BMW buildBMW(BMWModel model) {
    switch (model) {
      case E60:
        return new BMW(BMWModel.E60.getModel(), 3.2, FuelType.GASOLINE, 2017, steeringWheelPosition);
      case X5:
  ...
  public Ford buildFord(FordModel model) {
    switch (model) {
      case CMAX:
        return new Ford(FordModel.CMAX.getModel(), 1.6, FuelType.DIESEL, 2017, steeringWheelPosition);
      case FOCUS:
